...
describe('liveOnAirSceneBuilder', () => {
  ...
  it('デフォルトのデフォルト値は自動で付与される', () => {
    ...
    expect(_default.liveContext()).toEqual({})
    expect(_default.spot()).toHaveLength(1)
    expect(_default.spot()[0]).toMatchObject({ x: 0, y: 0, ...matchSpotAssets, name: '', labelFont: { size: 25 } })
    expect(_default.commentContext()).toEqual({})
    ...
    expect(_default.scorer()).toMatchObject({ font: { size: 40 }, digit: 4, prefix: 'スコア', suffix: '点', refrainsSendingScore: false })
  ...
  it('デフォルト要素が自動で定義される', () => {
    ...
    expect(_default.screen()).toEqual({})
    expect(_default.liveContext()).toEqual({})
    expect(_default.spot()).toMatchObject([{ x: 0, y: 0, ...matchSpotAssets, name: '', labelFont: { size: 25 } }])
    expect(_default.commentContext()).toEqual({})
    expect(_default.commentSupplier()).toMatchObject({ interval: 1000, comments: [] })
    expect(_default.commentDeployer()).toMatchObject({ speed: 4, intervalY: 40, font: { size: 35 } })
    expect(_default.scorer()).toMatchObject({ font: { size: 40 }, digit: 4, prefix: 'スコア', suffix: '点', refrainsSendingScore: false })
  ...
  it('デフォルト要素をカスタマイズできる', () => {
    _default
      ...
      .screen({ vars: 'screen' })
      .liveContext({ vars: 'lc' })
      .spot({ x: 500, y: 600, ...spotAssets, name: 'aaa', labelFont: new g.DynamicFont({ game: g.game, size: 50, fontFamily: 'sans-serif' }), vars: 'spot' })
      .commentContext({ vars: 'cc' })
      ...
      .scorer({ font: new g.DynamicFont({ game: g.game, fontFamily: 'sans-serif', size: 25 }), digit: 0, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: true, vars: 'scorer' })
    ...
    expect(_default.liveContext()).toEqual({ vars: 'lc' })
    expect(_default.spot()).toMatchObject([{ x: 500, y: 600, ...matchSpotAssets, name: 'aaa', labelFont: { size: 50 }, vars: 'spot' }])
    expect(_default.commentContext()).toEqual({ vars: 'cc' })
    ...
    expect(_default.scorer()).toMatchObject({ font: { size: 25 }, digit: 0, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: true, vars: 'scorer' })
    ...
    expect(sb.liveContext()).toEqual({ vars: 'lc' })
    expect(sb.spot()).toHaveLength(0)
    expect(sb.commentContext()).toEqual({ vars: 'cc' })
    ...
    expect(sb.scorer()).toMatchObject({ font: { size: 25 }, digit: 0, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: true, vars: 'scorer' })
  ...
  it('すべて未指定で定義した構成要素を参照できる', async () => {
    ...
    expect(sb.liveContext()).toEqual({})
    expect(sb.spot()).toMatchObject([{ x: 0, y: 0, ...matchSpotAssets, name: '', labelFont: { size: 25 } }, { x: 0, y: 0, ...matchSpotAssets, name: '', labelFont: { size: 25 } }])
    expect(sb.commentContext()).toEqual({})
    ...
    expect(sb.scorer()).toMatchObject({ font: { size: 40 }, digit: 4, prefix: 'スコア', suffix: '点', refrainsSendingScore: false })
    ...
    expect(s.screen.customLiveContext).toEqual({})
    ...
    expect(s.spots[0].name).toBe('')
    expect(s.spots[0].labelFont).toMatchObject({ size: 25 })
    expect(s.spots[1].location).toEqual({ x: 0, y: 0 })
    expect(s.spots[1].view).toMatchObject({ src: { path: './image/spot.default.unvisited.png' } })
    expect(s.spots[1].name).toBe('')
    expect(s.spots[1].labelFont).toMatchObject({ size: 25 })
    ...
    expect(s.scorer.refrainsSendingScore).toBe(false)
  ...
  it('定義したカスタム構成要素（すべて指定）を参照できる', async () => {
    const sb = new LiveOnAirSceneBuilder(g.game)
      ...
      .screen({ vars: 'screen' })
      .liveContext({ vars: 'lc' })
      .spot({ x: 700, y: 400, ...spotAssets, name: 'aaa', labelFont: new g.DynamicFont({ game: g.game, size: 50, fontFamily: 'sans-serif' }), vars: 'spot' })
      .commentContext({ vars: 'cc' })
      ...
      .scorer({ font: new g.DynamicFont({ game: g.game, fontFamily: 'sans-serif', size: 60 }), digit: 7, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: true, vars: 'scorer' })
    ...
    expect(sb.screen()).toEqual({ vars: 'screen' })
    expect(sb.liveContext()).toEqual({ vars: 'lc' })
    expect(sb.spot()).toMatchObject([{ x: 700, y: 400, ...matchSpotAssets, name: 'aaa', labelFont: { size: 50 }, vars: 'spot' }])
    expect(sb.commentContext()).toEqual({ vars: 'cc' })
    ...
    expect(sb.scorer()).toMatchObject({ font: { size: 60 }, digit: 7, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: true, vars: 'scorer' })
    ...
    expect(s.screen.customLiveContext).toEqual({ vars: 'lc' })
    expect(s.liveContext).toEqual({ vars: 'lc' })
    expect(s.spots).toHaveLength(1)
    expect(s.spots[0].location).toEqual({ x: 700, y: 400 })
    expect(s.spots[0].view).toMatchObject({ src: { path: './image/spot.default.unvisited.png' } })
    expect(s.spots[0].name).toBe('aaa')
    expect(s.spots[0].labelFont).toMatchObject({ size: 50 })
    expect(s.spots[0].vars).toBe('spot')
    expect(s.commentContext.vars).toBe('cc')
    ...
    expect(s.scorer.refrainsSendingScore).toBe(true)
  ...
  it('一度定義したカスタム構成要素を更新できる', () => {
    const sb = new LiveOnAirSceneBuilder(g.game)
      ...
      .scorer({ font: new g.DynamicFont({ game: g.game, fontFamily: 'sans-serif', size: 500 }), digit: 10, prefix: '￥', suffix: '円', refrainsSendingScore: true })
      .scorer({ font: new g.DynamicFont({ game: g.game, fontFamily: 'sans-serif', size: 30 }), digit: 2, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: false })
    ...
    expect(sb.scorer()).toMatchObject({ font: { size: 30 }, digit: 2, prefix: 'SCORE', suffix: 'pt', refrainsSendingScore: false })
