...
describe('commentSupplier', () => {
  ...
  it('コメントは1つ以上登録しないといけない', () => {
    expect(() => new CommentSupplierBuilder(g.game).build()).toThrow()
  })
  it('条件なしコメントを登録できる', () => {
    const cs = new CommentSupplierBuilder(g.game)
      .add('hoge')
      .build()
  ...
  it('コメント率を設定できる', () => {
    const cs = new CommentSupplierBuilder(g.game).add('hoge').build()
    cs.interval = 0.1
  ...
  it('無効なfps値は設定できない', () => {
    g.game.fps = -1
    expect(() => new CommentSupplierBuilder(g.game).add('hoge').build()).toThrow()
  })
})
